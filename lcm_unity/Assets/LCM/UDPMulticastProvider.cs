	...
	public class UDPMulticastProvider : Provider
	{
		...
		public void Subscribe(string channel)
		{
			lock (this)
			{
				if (!reader_started) 
				{
					Debug.Log (LCMCoroutineHelper.SINGLETON);
					LCMCoroutineHelper.SINGLETON.StartCoroutine(ReaderCoroutineRun());
					reader_started = true;
				}
			}
		}
		...
		public void Close()
		{
			...
			sock = null;
			fragBufs = null;
			LCMCoroutineHelper.SINGLETON.StopAllCoroutines();
		}
		...
		private System.Collections.IEnumerator ReaderCoroutineRun() {
			byte[] packetData;
